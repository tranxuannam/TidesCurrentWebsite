...
class Simple_Posts_Table {
	...
	public static $default_args = array(
		'columns' => 'title,content,date,author,category,latitude,longitude',
		'rows_per_page' => 20,
		'sort_by' => 'date',
		'sort_order' => '',
		'category' => '',
	...
	public static function get_column_defaults() {
		if ( empty( self::$column_defaults ) ) {
			...
			self::$column_defaults = array(
				...
				'latitude' => array(
					'heading' => __( 'Latitude', 'posts-data-table' ),
					'priority' => 8,
					'width' => ''
				),
				'longitude' => array(
					'heading' => __( 'Longitude', 'posts-data-table' ),
					'priority' => 9,
					'width' => ''
				),
				'code' => array(
					'heading' => __( 'Code', 'posts-data-table' ),
					'priority' => 10,
					'width' => ''
				)
			);
	...
	public function get_table( $args ) {
		...
		$post_args = array(
			'post_type' => 'post',
			'posts_per_page' => apply_filters( 'posts_data_table_post_limit', 9124 ),
			'post_status' => 'publish',
		...
		foreach ( (array) $all_posts as $_post ) {
			...
			$post_data_trans = apply_filters( 'posts_data_table_row_data_format', array(
				...
				'{latitude}' => get_post_meta( $_post->ID, 'latitude', true ),
				'{longitude}' => get_post_meta( $_post->ID, 'longitude', true ),
				'{code}' => get_post_meta( $_post->ID, 'code', true )
				) );
